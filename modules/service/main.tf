...
locals {
  ...
  is_codedeploy          = try(var.deployment_controller.type, null) == "CODE_DEPLOY"
...
resource "aws_ecs_service" "this" {
  count = local.create_service && !var.ignore_task_definition_changes && !local.is_codedeploy ? 1 : 0
...
################################################################################
# Service - Ignore some resources for CodeDeploy
################################################################################

resource "aws_ecs_service" "codedeploy" {
  count = local.create_service && local.is_codedeploy ? 1 : 0
...
resource "aws_appautoscaling_target" "this" {
  ...
  resource_id        = "service/${local.cluster_name}/${try(aws_ecs_service.this[0].name, aws_ecs_service.ignore_task_definition[0].name, aws_ecs_service.codedeploy[0].name)}"
